...
func mustEnvStr(key string) string {
	if value, ok := os.LookupEnv(key); ok {
		...
		logging.Default.Panicf("Environment variable %v must be set.", key)
		return ""
	}
}
...
func optionalEnvBool(key string, fallback bool) bool {
	if strValue, ok := os.LookupEnv(key); ok {
		...
		if err != nil {
			logging.Default.Panicf("Failed to parse environment variable as bool %v with value %v.", key, strValue)
			return fallback
		}
		return boolValue
	}
...
func optionalEnvInt(key string, fallback int) int {
	if strValue, ok := os.LookupEnv(key); ok {
		...
		if err != nil {
			logging.Default.Panicf("Failed to parse environment variable as int %v with value %v.", key, strValue)
			return fallback
		}
		return intValue
	}
...
func optionalEnvInt64(key string, fallback int64) int64 {
	if value, ok := os.LookupEnv(key); ok {
		...
		if err != nil {
			logging.Default.Panicf("Failed to parse environment variable as int64 %v with value %v.", key, value)
			return fallback
		}
		return int64Value
	}
